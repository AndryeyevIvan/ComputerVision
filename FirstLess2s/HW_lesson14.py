...
count_people_images = 0
count_no_people_images = 0
...
for filename in files:
    ...
    boxes, confidences = detect_people_on_image(img)
    ...
    if people_count > 0:
        ...
        cv2.imwrite(os.path.join(PEOPLE_DIR, "boxed_" + filename), boxed)
        count_people_images += 1
        print(f"[PEOPLE] {filename}  count={people_count}")
    else:
        shutil.copy2(in_path, os.path.join(NO_PEOPLE_DIR, filename))
        count_no_people_images += 1
        print(f"[NO]     {filename}")

print("\nГотово!")
print("З людьми:", count_people_images)
